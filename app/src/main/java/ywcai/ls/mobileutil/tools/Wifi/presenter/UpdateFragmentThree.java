...
public class UpdateFragmentThree implements UpdateFragmentThreeInf {
    ...
    @Override
    public void refreshChannelCurrent(int[] channelSum) {
        int[] sum;
        ...
        sendMsgRefreshChart(sum);
    }



    @Override
    public void refreshChannelRecord(int[] channelSum) {

    }

    @Override
    public void refreshChannelForPieChart(int[] channelSum) {
        sendMsgRefreshPieChart(channelSum);
    }


    private void sendMsgRefreshChart(int[] channelSum) {
        MsgHelper.sendEvent(GlobalEventT.wifi_refresh_three_bar, wifiState.choose2d4G+"", channelSum);
    }
    private void sendMsgRefreshPieChart(int[] allChannelSum) {
        MsgHelper.sendEvent(GlobalEventT.wifi_refresh_three_pie, "", allChannelSum);
    }
