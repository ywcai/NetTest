...
public class YibiaoView extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (isShowProgress) { // 显示实时读数，View高度增加字体高度3倍
...
//        );
        int canvasHeight = (int) (point1[1] + mStrokeWidth * 2 + mRadius / 5);
...
//        }
         ...
         * 画指针围绕的镂空圆心
         */
        mPaint.setColor(ContextCompat.getColor(getContext(), R.color.card_background));
        canvas.drawCircle(mCenterX, mCenterY, dp2px(2), mPaint);

        /**
         * 画实时度数值
         */
        if (isShowProgress && progress >= 0) {
    ...
    public void setRealTimeValue(float realTimeValue) {
        ...
        if (realTimeValue < mMin) {
            mRealTimeValue = mMin;
            invalidate();
            return;
        }
        if (realTimeValue > mMax) {
            mRealTimeValue = mMax;
            invalidate();
            return;
        }
